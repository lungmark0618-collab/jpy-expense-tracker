...
with st.expander("💡 第一次使用？點我查看操作教學"):
    st.markdown("""
    **✅ 如何快速記帳？**
    1. 在畫面上方選擇「📌 快速分類」或輸入「📝 細項備註」（例如：拉麵）。
    2. 使用下方的計算機輸入日幣金額（支援加減乘除，例如：`1000 + 500`）。
    3. 按下大大的 **「=」** 鍵，金額就會自動換算台幣，並存入最下方的「🧾 購物明細」中！
    ...
    **✅ 如何跟朋友分帳？**
    - 點擊切換到最下方的 **「👥 分帳小幫手」** 分頁。
    - 輸入你們的人數，系統會抓取總金額，自動算出每個人該付多少台幣/日幣。
    - 點擊「💬 產生 LINE 請款訊息」，就可以直接複製文字貼去群組收錢囉！
    
    **⚠️ 常見問題：重整網頁資料會不見嗎？**
    目前這是一個輕量的暫存工具，**重新整理網頁會清空所有資料**。建議在結束一天的行程、或是吃完一頓飯後，就點擊下方的「📋 複製純文字明細」把資料貼到你手機的備忘錄裡保存喔！
    """)
